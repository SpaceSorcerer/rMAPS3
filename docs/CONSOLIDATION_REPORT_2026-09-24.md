...
**Current HEAD (v1.0.1d, updated 2026-09-24).** In the lock environment (`E:\rmaps_venv2`, built from `requirements-lock.txt`) pytest collects **336 tests** with `python -m pytest --collect-only -q tests --ignore=tests/legacy --ignore=tests/test_motif.py`; `tests/test_release_docs.py` fails if this number and the collection disagree. Full suite on 2026-09-24, v1.0.1d code: 336 passed, 0 skipped in `E:\rmaps_venv2`; 331 passed, 3 skipped in `E:\rmaps_venv`, which lacks pandas and PyYAML. Everything below this paragraph is the record of the consolidation at `fe7d32b` and its numbers belong to that commit.
