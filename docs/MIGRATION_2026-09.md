...
| Non-empty output refused (AUDIT S1) | Use a fresh directory or `--overwrite` with a prior manifest; no-manifest reuse and unregistered target collisions are refused before writes. Previous registered retained outputs plus manifest move to `_previous_<UTC timestamp>/`. Unrelated unowned files are preserved. |
...
| Retained FASTA crops (AUDIT S4) | Retained FASTA sequences now contain region-plus-halo crops rather than full exon/intron features. The halo is the largest supported motif maximum match width across both motif inputs minus one nucleotide on each side, clipped to feature and chromosome bounds. Consumers must not interpret these sequences as full features. |
