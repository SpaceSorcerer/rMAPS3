...
- **Calibration v2.1 (2026-09-22) is the reportable supplement.** It is produced by `tools/calibrate_ranksum_v2.py`.
  ...
  - There are two stages. Stage 1 runs 2,000 permutations for every test. Stage 2 runs 100,000, and a test reports it only when its own stage-1 p ≤ 0.005. The stage streams are independent, and the seed is 149.
...
- **Two-stage validity (2026-09-24, v1.0.1b: self-triggered promotion).** Every calibrated p is a valid permutation p. Each test reports its stage-2 p (B = 100,000, resolution 1/100,001) only when its own stage-1 p (B = 2,000, resolution 1/2,001) is ≤ t = 0.005; otherwise it keeps its stage-1 p. `calib_perms_used` gives each p's B. Proof and scope: [`docs/two_stage_validity.md`](docs/two_stage_validity.md).
  - Each stage p is (b+1)/(B+1) over exchangeable cluster draws, so each is super-uniform.
  - For α ≤ t, P(p ≤ α) ≤ P(p2 ≤ α) ≤ α. For α > t, P(p ≤ α) ≤ P(p1 ≤ t) + P(t < p1 ≤ α) = P(p1 ≤ α) ≤ α. Neither bound needs p1 and p2 to be independent; they share the observed data.
  - **v1.0.1 promoted by group and was not valid.** A pair or RBP promoted by one statistic, pool or motif switched every test in it to stage 2, including tests whose own stage-1 p was above t. For α > t that lets a test with p1 > α land below α. `tests/test_two_stage_fdr.py` constructs this failure and checks the arm runner never switches such a test. Stage-2 draws may still run for a whole pair or RBP; only the switch is per test.
  - BH over super-uniform p controls the FDR under PRDS-type dependence. PRDS is assumed, not proven: shared permutations within an arm and direction, overlapping k-mers and the shared background all induce dependence. The test suite checks FDR on global-null arms with correlated motifs.
